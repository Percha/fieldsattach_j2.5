...
class fieldsattachHelper
{
	 ...
	 */
		...
		fieldsattachHelper::writeLog("uploadFile: ".$path);
...
/*
	...
	*/
	    ...
	    $language = JRequest::getVar("select_language_id");
	    if(!empty($language))
	    {
		$result ="";
		$db	= & JFactory::getDBO();
		$query = 'SELECT a.value FROM #__fieldsattach_falang as a WHERE a.fieldsid='. $fieldsid.' AND a.articleid='.$articleid.' AND a.language='.$language ;
		//echo $query;
		$db->setQuery( $query );
		$elid = $db->loadObject();
		...
		//$logs = $this->params->get('logs', '');
                //if($logs){
                    ...
                    JLog::addLogger(
                        array(
                             //Sets file name
                             'text_file' => 'com_fieldsattach.log'
                        ),
                    ...
                    JLog::add($mesg, JLog::WARNING, 'com_fieldsattach');
                    //LOG*********
               // }
	}
