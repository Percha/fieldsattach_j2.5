...
*/
                ...
                foreach($nameslst as $obj)
                {
                    ...
                    if(count($valueslst)==0)
                        ...
                        if(!empty($results)){
                            ...
                            //eval($function);
			    $tmp = call_user_func_array(array("plgfieldsattachment_".$obj->type."::action"),array($article->id,$obj->id, $valueslst->id));
	...
	*/
	...
	{  
            ...
            if( ($app->isAdmin() && JRequest::getVar("id") != $article->id && (!empty( $article->id))   && ($article->id>0) && (JRequest::getVar("id")>0))  ){
	...
	function cleandatabase()
	{
		$option = JRequest::getCmd('option', '');
                ...
                { 
                        ...
                        $query = 'DELETE FROM #__content WHERE title="" AND created_by='.$user->get('id').'';
	...
	*/
	function onAfterRenderCategory()
	{
            ...
            if(count($fields)>0){
				...
				$sustitucion = call_user_func_array(array("plgfieldsattachment_".$obj->element."::getHTML"),array($id, $field->id, true));
...
*/
	...
	*/
	public function copyArticle($article,$oldid)
        {
		fieldsattachHelper::writeLog("CopyArticle: ".$article->id." OLD:".$oldid);
	...
	public function copygallery($oldid,$newid)
	{
		 $db	= & JFactory::getDBO();
		//COPY  fieldsattach_images GALLERIES-----------------------------
                $query = 'SELECT * FROM #__fieldsattach_images as a  WHERE a.articleid = '. $oldid ;
		
		fieldsattachHelper::writeLog("QUERY: ".$query);
                 
                $db->setQuery( $query );
                $results= $db->loadObjectList();
		fieldsattachHelper::writeLog("count: ".count($results));
                if(count($results)>0){
