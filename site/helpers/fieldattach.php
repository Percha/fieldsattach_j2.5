...
class fieldattach
{
	...
	public function getValue($articleid, $fieldsids, $category = false )
	{
	    ...
	    {
		//GEt Idlanguage
		$query = 'SELECT a.lang_id FROM #__languages as a WHERE a.lang_code="'. $language.'"' ;
		 
		$db->setQuery( $query );
		$objlang = $db->loadObject();
		
		if(!empty($objlang->lang_id))
		{
		    $result ="";
		    $language = $objlang->lang_id;
		    $query = 'SELECT a.value FROM #__fieldsattach_falang as a WHERE a.fieldsid IN ('.$fieldsids.') AND a.articleid='.$articleid.' AND a.language='.$language ;
